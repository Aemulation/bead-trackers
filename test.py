...
for j in range(num_iters + num_warmup):
    ...
    print(
        f"{j}/{num_iters + num_warmup}: multi threaded elapsed: {(end - start) * 1_000}ms "
    )
...
for j in range(num_iters + num_warmup):
    ...
    print(
        f"{j}/{num_iters + num_warmup}: threadpool elapsed: {(end - start) * 1_000}ms "
    )
...
for j in range(num_iters + num_warmup):
    ...
    print(
        f"{j}/{num_iters + num_warmup}: multiprocessing elapsed: {(end - start) * 1_000}ms "
    )
...
for j in range(num_iters + num_warmup):
    ...
    print(
        f"{j}/{num_iters + num_warmup}: single threaded elapsed: {(end - start) * 1_000}ms "
    )
